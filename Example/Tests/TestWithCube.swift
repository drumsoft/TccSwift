...
fileprivate func testResult<Type>(_ result:Result<Type,Error>) {
    switch result {
    case .success(_):
        break // ok
...
fileprivate func testResult<Type,T>(_ result:Result<Type,Error>, as type:T.Type) {
    switch result {
    case .success(let r):
...
class TestWithCube_Basic: QuickSpec {
    private class MyCubeManagerDelegate: CubeManagerDelegate {
        var onFound:(()->())?
    ...
    override func spec() {
        ...
        print("\"Leave it\" Test: Power on and leave a Core Cube on your desk to start the test.")
        
        describe("CubeManager") {
            let cubeManager = CubeManager()
            let cubeManagerDelegate = MyCubeManagerDelegate()
